    ...
    <div className="card space-y-4">
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Avoid faces/plates. You consent to civic reporting.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Name (for leaderboard)</label>
            <input className="w-full border rounded px-3 py-2" value={reporter} onChange={e => setReporter(e.target.value)} placeholder="Anonymous" />
          </div>
        </div>
      ...
      {result && (
        <div className="border rounded-lg p-3 bg-gray-50">
          ...
          <div className="mt-2">
            <a className="text-blue-600 underline" href={result.image_url} target="_blank" rel="noreferrer">Open uploaded image</a>
          </div>
        </div>
      )}
    </div>
